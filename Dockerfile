...
RUN apt-get update -qq \
    ...
    && curl -fL https://surfer.nmr.mgh.harvard.edu/pub/dist/freesurfer/7.1.1/freesurfer-linux-centos8_x86_64-7.1.1.tar.gz \
...
RUN apt-get update -qq \
           && apt-get install -y -q --no-install-recommends \
                  nodejs \
           && rm -rf /var/lib/apt/lists/*
RUN bash -c 'npm install -g bids-validator'
...
RUN printf '{ \
  ...
  "instructions": [ \
    ...
    { \
      "name": "run", \
      "kwds": { \
        "command": "bash -c '"'"'curl -sL https://deb.nodesource.com/setup_14.x | bash -'"'"'" \
      } \
    }, \
    { \
      "name": "install", \
    ...
    { \
      "name": "run", \
      "kwds": { \
        "command": "bash -c '"'"'npm install -g bids-validator'"'"'" \
      } \
    }, \
    { \
      "name": "env", \
